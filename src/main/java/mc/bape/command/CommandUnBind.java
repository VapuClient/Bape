...
public class CommandUnBind implements ICommand {

	private Minecraft mc = Minecraft.getMinecraft();

	public Client BapeClient =null;

	public CommandUnBind(Client vapuClient) {
		...
		if (args.length == 1 && !ModuleManager.getModule("NoCommand").getState()) {
			try {
				...
				if(Module == null){
					...
					Module.setKey(KEY_NONE);
					Helper.sendMessage(Module.getName()+" bind has been deleted");
				}
			}
