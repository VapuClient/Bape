...
public class CommandConfig implements ICommand {
	...
	public CommandConfig(Client vapuClient) {
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		if (!ModuleManager.getModule("NoCommand").getState()) {
			try {
				...
				ConfigManager.ConfigFile = "/" + names + "/";
				List<String> binds = ConfigManager.read(names +"-binds.cfg");
				for (String v : binds) {
					ConfigManager.ConfigFile = "/" + names + "/";
				...
				List<String> enabled = ConfigManager.read(names +"-modules.cfg");
				...
				List<String> vals = ConfigManager.read(names +"-values.cfg");
				for (String v : vals) {
					ConfigManager.ConfigFile = "/" + names + "/";
					String name = v.split(":")[0];
