	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		if (args.length == 2 && !ModuleManager.getModule("NoCommand").getState()) {
			try {
				...
				if(Module == null){
					ClientUtil.sendClientMessage("Module not found. please use correct module name.", Notification.Type.ERROR);
					// Helper.sendMessage("Module not found. please use correct module name.");
				} else {
					Module.setKey(Keyboard.getKeyIndex(key.toUpperCase()));
					ClientUtil.sendClientMessage("bind "+Module.getName()+" to "+key, Notification.Type.SUCCESS);
