...
public class ClickGUI extends GuiScreen {
    ...
    private List<mc.bape.gui.LiquidBaseClickGui.UI.Panel> panels = new ArrayList<>();

    public ClickGUI() {
        int x = 20;

        for(final ModuleType moduleCategory : ModuleType.values()) {
            final mc.bape.gui.LiquidBaseClickGui.UI.Panel panel = new mc.bape.gui.LiquidBaseClickGui.UI.Panel(moduleCategory.getName(), x, 50, 100);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        // DO NOT DELETE THIS!
        fontRendererObj.drawStringWithShadow(Client.CLIENT_NAME + Client.CLIENT_VERSION +" Author: Ceeyourbac & mckuhei Credits: Medit_4", 5, this.height - 20, new Color(255, 255, 255).hashCode());
        ...
        for(final mc.bape.gui.LiquidBaseClickGui.UI.Panel panel : panels) {
            ...
            for(int i = 0; i < panel.getButtons().size(); i++) {
                ...
                if(Client.STRING_GOD_DETECTION){
                    try {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if(mouseButton == 0) {
            for(int index = panels.size() - 1; index >= 0; index--) {
                final mc.bape.gui.LiquidBaseClickGui.UI.Panel panel = panels.get(index);
